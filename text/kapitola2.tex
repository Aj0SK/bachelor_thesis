...
If the number of hits of the positive squiggle is $0$ or the ratio exceeds 2:1, then this ratio is undefined
so we mark this ratio as a 2:1.

Figure \ref{obr:res_3} shows the results of the experiment with using all the smoothing
...
We see that there are parameters that obtain the ratio at most $1/2$ for more than 80\% of test cases
...
results for the different number of levels. Individual curve represent different
number of $k$-mers.}
\label{obr:res_3}
\end{figure}
...
If the two positive squiggles have
...
due to some properties of the discretization algorithm. 
...
techniques applied, only the lower lengths of $k$-mers have meaningful results ($k$-mer length $\geq 17$)
...
match due to the reasons outlined in the Section \ref{subsection:oscillations}.
...
minimizing the number of the gaps. While in previous experiments we focus on
...
We split the gaps into the two groups according
