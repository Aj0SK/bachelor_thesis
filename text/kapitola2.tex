...
simulated reference and the unrelated squiggle. For easier description, we will
label the simulated reference squiggle the reference, real squiggle that corresponds
to this reference the positive squiggle and the random squiggle the negative squiggle. 
We look at what is the usual ratio of the hits of negative squiggle to the
hits of the positive squiggle. We want this number to be as big as possible. We will
track only up to ratio 1:2. If the number of hits of the positive squiggle is $0$
then we will mark this as a ratio 1:2. The goal of this experiment is to see how informative
is the number of shared $k$-mers regarding the underlying squiggles.
The results of the experiment with using all the smoothing methods that we introduces are on Figure \ref{obr:res_3}.
In this figure, we see the cummulative ratio of the measured ratios.
We see that there are parameters that obtain the 25\% ratio for more than 80\% of testcases. 
...
high $k$ but it is also hard for the real squiggles to obtain
high number of hits so for a lot of reads the number of hits of the positive squiggle is 0.
...
techniques applied the lower lengths of $k$-mers have better results. We think that this is
