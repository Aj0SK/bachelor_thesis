...
Change to the parameter $w$ balance specificity and informativness of our method.
...
We now compare the simulated squiggle to real squiggle that arose from the real
sequencing. Moreover, we do not want to compare our simulated squiggle with unrelated squiggle
but with the squiggle that corresponds to the same DNA region.

When we want to compare the real and simuleted signal, we first take some real squiggle.
Then, we find the part of the DNA reference that this squiggle corresponds. After
successfully finding this part of the DNA reference we simulate the squiggle from it. There are two
...
In Figure \ref{obr:testSmooth} we can see the part of the simulated and corresponding real squiggle.
...
We take the real squiggle, the squiggle simulated from it and unrelated real squiggle.
We will then try to distinguish which squiggle is from the reference
...
\begin{itemize}
    \item We are interested in how accurately the discretization method that we proposed in Section \ref{section:squigglesAsDiscrete} represents the squiggle.
    \item We want to test how the proposed identification from Section \ref{section:readIdentification} works on real data
\end{itemize}

In both of these cases we are interested in the results for the different parameters
as the number of levels, length of the $k$-mers and impact of the methods that we
...
Note that only 12 reads out of 500 in case of the \emph{Saprochaete fungicola} had more than 50\% of their length aligned.
...
work only with the first $5\,000$ readouts from the segment that Nadavca identified
...
In Figure \ref{obr:res_2} we see the ROC curve that represents how the overall number
