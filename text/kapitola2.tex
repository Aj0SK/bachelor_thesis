...
The idea of our solution is to somehow discretize the simulated signal of the
reference sequence and then be able to decide during the process of sequencing
if the DNA molecule currently passing through the pore is from this reference signal.
In our current setup, we are not very limited by the preprocessing time but we
need to decide very fast once we are sequencing the DNA. This puts some speed
limitations on our discretizing process during the processing of the DNA.

We want to create a fast yet accurate solution. We want to come with some better
representation of the squiggles that represents signal well and also provides some
advantages such as easy searching of the signal in this representation. It is good
for people to represent the signal visually as a graph. To a certain extent, we
can see that the two signals are similar. However, we need to do this fast and on very
long signals. 

The main goal is to be able to find the signal in the reference
...
If the signals $a_0, a_1 \cdots a_n$ are in the windows $w_0, w_1 \cdots w_i$
we call $l=w_1w_2\cdots w_n$ the level string of the signal $s$. An example of
this method on some signal for $w=6$ is on TODO. This method has
...
measured several times. We already estimated that one nucleotide is on average
