...
is very good as it can be easily represented in the computer and also stores enough
information as we can recover the other complementary DNA string easily. The process of obtaining DNA
...
/\textit{raw signal} or as we will call it \textit{squiggle}. The example of this
signal is on figure \ref{obr:minIonCurrent}.
...
that it produces quite long reads. The length of the read is stated in kilo-bases
...
The continuous sequence of $k$ nucleotides is called \textit{k-mer}. There are
several studies how long kmer really influence output signal. Usually, with the MinION
comes something what is called a kmer model. It is a list of all possible $k$-mers
...
of DNA looked like. Despite big improvements in the base-calling algorithms,
the overall process of base-calling is quite slow and resource-intensive. The early
basecalling algorithms tried to split the signal into events, some kind of longer
signals at the same level corresponding to one particular $k$-mer. Now, the more
successful methods are based on recurrent neural networks.
